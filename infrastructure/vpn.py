...
def activateVpn(options):
    subprocess.run(['protonvpn-cli', 'c'] + options, stdout=subprocess.PIPE).stdout.decode('utf-8')

def deactivateVpn():
    subprocess.run(['protonvpn-cli', 'd'], stdout=subprocess.PIPE).stdout.decode('utf-8')

def getVpnStatus():
    nets = subprocess.Popen(('nmcli', 'c', 'show', '--active'), stdout=subprocess.PIPE)
    vpns = subprocess.run(('grep', '-i', 'vpn'), stdin=nets.stdout, stdout=subprocess.PIPE)
    nets.wait()
    if vpns.stdout == None:
        return 'disconnected'
