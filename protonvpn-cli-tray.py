...
class SystemTrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def addActionsMultiSelect(self, actionsAndValues, settingsKey):
        for action, value in actionsAndValues:
            ...
            action.triggered.connect(
                lambda state, x=value: self.updateSettingsAndActions(
                    settingsKey, x, actionsAndValues
                )
            )
    ...
    def __init__(self, parent=None):
        ...
        self.addActionsMultiSelect(
            [[setProtocolUdp, "udp"], [setProtocolTcp, "tcp"]], "protocol"
        )
        menu.addSeparator()
    ...
    def activateAction(self):
        activateVpn(
            [self.settings["serverConnectionType"], "-p", self.settings["protocol"]]
        )
